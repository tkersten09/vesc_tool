...
#include "pageinputsettings.h"
#include "ui_pageinputsettings.h"
#include "utility.h"

PageInputSettings::PageInputSettings(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::PageInputSettings)
{
    ui->setupUi(this);
...
PageInputSettings::~PageInputSettings()
{
    delete ui;
}

VescInterface *PageInputSettings::vesc() const
{
    return mVesc;
}

void PageInputSettings::setVesc(VescInterface *vesc)
{
    mVesc = vesc;

    if (mVesc) {
        ui->generalTab->addParamRow(mVesc->appConfig(), "app_to_use");
        ui->generalTab->addParamRow(mVesc->appConfig(), "controller_id");
        ui->generalTab->addParamRow(mVesc->appConfig(), "timeout_msec");
        ui->generalTab->addParamRow(mVesc->appConfig(), "timeout_brake_current");
        ui->generalTab->addParamRow(mVesc->appConfig(), "send_can_status");
        ui->generalTab->addParamRow(mVesc->appConfig(), "send_can_status_rate_hz");
        ui->generalTab->addParamRow(mVesc->appConfig(), "can_baud_rate");
        ...
        ui->newTab->addRowSeparator(tr("New Tab"));
        ui->newTab->addParamRow(mVesc->appConfig(), "app_ppm_conf.pulse_start");
        ui->newTab->addParamRow(mVesc->appConfig(), "app_ppm_conf.pulse_end");
        ui->newTab->addParamRow(mVesc->appConfig(), "app_ppm_conf.pulse_center");
        ui->newTab->addParamRow(mVesc->appConfig(), "app_ppm_conf.hyst");
...
void PageInputSettings::paramChangedDouble(QObject *src, QString name, double newParam)
{
    (void)src;
    (void)newParam;
...
void PageInputSettings::paramChangedEnum(QObject *src, QString name, int newParam)
{
    (void)src;
    (void)newParam;
